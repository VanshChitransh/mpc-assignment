...
## ✅ Step 3.1: Wallet-Specific REST APIs
...
## ✅ Step 3.2: Backend Orchestration & State Management
...
## ✅ Step 3.3: API Layer & External Integration
...
1. **POST /api/v1/wallet/keygen**
2. **POST /api/v1/wallet/sign/phase1**
3. **POST /api/v1/wallet/sign/phase2**
4. **POST /api/v1/wallet/aggregate**
5. **GET /api/v1/wallet/health**
...
```bash
# Create session
SESSION_RESPONSE=$(curl -X POST http://localhost:8080/api/v1/wallet/sign/phase1 \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
...
# Wait for session to expire (adjust timeout in config)
...
curl -X POST http://localhost:8080/api/v1/wallet/sign/phase2 \
  ...
  -H "Content-Type: application/json" \
  -d '{"session_id": "'$SESSION_ID'", "message": "test message"}'
# Expected: 410 Gone - Signing session expired
```
...
```bash
# Simulate temporary network issues by stopping/starting nodes
pkill -f "mpc.*node1"
sleep 5
./start_mpc_cluster.sh  # Restart cluster
...
curl -X POST http://localhost:8080/api/v1/wallet/keygen \
  ...
  -H "Content-Type: application/json" \
  -d '{"threshold": 2, "participants": 3}'
# Expected: 200 OK after retry attempts
```
...
if (( $(echo "$RESPONSE_TIME < 5.0" | bc -l) )); then
  ...
  echo "❌ Latency test failed: ${RESPONSE_TIME}s"
fi
```
