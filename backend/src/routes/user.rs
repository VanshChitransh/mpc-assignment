    ...
    match store.create_user(create_request).await {
        Ok(user) => {
            ...
            let user_uuid = match Uuid::parse_str(&user.id) {
                Ok(uuid) => uuid,
                Err(e) => {
                    error!("Failed to parse user ID as UUID: {} - Error: {}", user.id, e);
                    return Ok(HttpResponse::InternalServerError().json(ErrorResponse {
                        success: false,
                        error: "Internal error processing user ID".to_string(),
                    }));
                }
            };
        ...
        Ok(user) => {
            ...
            let user_uuid = match Uuid::parse_str(&user.id) {
                ...
                Err(e) => {
                    ...
                    return Ok(HttpResponse::InternalServerError().json(ErrorResponse {
                        success: false,
                        error: "Internal error processing authentication".to_string(),
                    }));
                }
            };
